...
### 1. Média de Y ####
# Controle Sintético
cs_InvPriv_NivelMediaY <- dfNivel %>% 
  ...
  generate_predictor(time_window = nAnoInicial:nAnoTratamento,
   ...
   generate_predictor(time_window = nAnoInicial:nAnoTratamento,
                      `Investimento Privado` = mean(Investimento_Privado_PIB_PPP, na.rm = T)) %>% 
   generate_weights(optimization_window = nAnoInicial:nAnoTratamento) %>% 
   generate_control()

# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_NivelMediaY <- funcs$mspe_placebos_pos_trat(cs_InvPriv_NivelMediaY,
                                                         nAnoTratamento))
(mspe_pre_br_NivelMediaY <- cs_InvPriv_NivelMediaY %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))
...
lista_graficosMediaY <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPriv_NivelMediaY, 
...
funcs$tabela_media_cs(cs_InvPriv_NivelMediaY, stargazer = F)
...
cs_InvPriv_NivelMediaY %>% grab_signficance()

### 2. Todos os Valores de Y ####
# Controle Sintético
cs_InvPriv_Nivel_ValoresY <- dfNivel %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_Privado_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
  ...
  generate_predictor(time_window = nAnoInicial:nAnoTratamento,
   ...
   generate_predictor(time_window = 2005,
                      `Investimento Privado 2005` = Investimento_Privado_PIB_PPP) %>%
   generate_predictor(time_window = 2006,
                      `Investimento Privado 2006` = Investimento_Privado_PIB_PPP) %>% 
   generate_predictor(time_window = 2007,
                      `Investimento Privado 2007` = Investimento_Privado_PIB_PPP) %>% 
   generate_predictor(time_window = 2008,
                      `Investimento Privado 2008` = Investimento_Privado_PIB_PPP) %>% 
   ...
   generate_predictor(time_window = 2011,
                      `Investimento Privado 2011` = Investimento_Privado_PIB_PPP) %>% 
...
# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_Nivel_ValoresY <- funcs$mspe_placebos_pos_trat(cs_InvPriv_Nivel_ValoresY,
                                                         nAnoTratamento))
(mspe_pre_br_Nivel_ValoresY <- cs_InvPriv_Nivel_ValoresY %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))
...
lista_graficos_ValoresY <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPriv_Nivel_ValoresY, 
   nome_variavel_y = "Investimento Privado Real PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
...
funcs$tabela_media_cs(cs_InvPriv_Nivel_ValoresY, stargazer = F)
...
cs_InvPriv_Nivel_ValoresY %>% grab_signficance()

### 3. Inflexões ####
# Controle Sintético
cs_InvPriv_Nivel_Inflexoes <- dfNivel %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_Privado_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
...
# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_Nivel_Inflexoes <- funcs$mspe_placebos_pos_trat(cs_InvPriv_Nivel_Inflexoes, 
                                                          nAnoTratamento))
(mspe_pre_br_Nivel_Inflexoes <- cs_InvPriv_Nivel_Inflexoes %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))
...
lista_graficos_Inflexoes <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPriv_Nivel_Inflexoes, 
   nome_variavel_y = "Investimento Privado Real PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
...
funcs$tabela_media_cs(cs_InvPriv_Nivel_Inflexoes, stargazer = F)
...
cs_InvPriv_Nivel_Inflexoes %>% grab_signficance()


### 4. Primeiros 75% dos Valores de Y ####
cs_InvPriv_Nivel_TresQuartosValoresY <- dfNivel %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_Privado_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
...
(mspe_pos_Nivel_TresQuartosValoresY <- funcs$mspe_placebos_pos_trat(
   cs_InvPriv_Nivel_TresQuartosValoresY, nAnoTratamento
))
(mspe_pre_br_Nivel_TresQuartosValoresY <- cs_InvPriv_Nivel_TresQuartosValoresY %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))
...
lista_graficos_TresQuartosValoresY <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPriv_Nivel_TresQuartosValoresY, 
   nome_variavel_y = "Investimento Privado Real PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
...
funcs$tabela_media_cs(cs_InvPriv_Nivel_TresQuartosValoresY, stargazer = F)
...
cs_InvPriv_Nivel_TresQuartosValoresY %>% grab_signficance()

### 5. Últimos 25% dos Valores de Y ####
cs_InvPriv_Nivel_UltQuartoValoresY <- dfNivel %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_Privado_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
...
(mspe_pos_Nivel_UltQuartoValoresY <- funcs$mspe_placebos_pos_trat(
   cs_InvPriv_Nivel_UltQuartoValoresY, nAnoTratamento
))
(mspe_pre_br_Nivel_UltQuartoValoresY <- cs_InvPriv_Nivel_UltQuartoValoresY %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))
...
lista_graficos_UltQuartoValoresY <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPriv_Nivel_UltQuartoValoresY, 
   nome_variavel_y = "Investimento Privado Real PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
...
funcs$tabela_media_cs(cs_InvPriv_Nivel_UltQuartoValoresY, stargazer = F)
...
cs_InvPriv_Nivel_UltQuartoValoresY %>% grab_signficance()


### 6. Apenas Covariadas Externas ####
cs_InvPriv_Nivel_Controles <- dfNivel %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_Privado_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
...
# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_Nivel_Controles <- funcs$mspe_placebos_pos_trat(cs_InvPriv_Nivel_Controles, 
                                                          nAnoTratamento))
(mspe_pre_br_Nivel_Controles <- cs_InvPriv_Nivel_Controles %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))
...
lista_graficos_Controles <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPriv_Nivel_Controles, 
   nome_variavel_y = "Investimento Privado Real PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
...
funcs$tabela_media_cs(cs_InvPriv_Nivel_Controles, stargazer = F)
...
cs_InvPriv_Nivel_Controles %>% grab_signficance()


### 7. Apenas os Valores de Y ####
# Controle Sintético
cs_InvPriv_Nivel_ApenasValoresY <- dfNivel %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_Privado_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
...
(mspe_pos_Nivel_ApenasValoresY <- funcs$mspe_placebos_pos_trat(
   cs_InvPriv_Nivel_ApenasValoresY,nAnoTratamento
))

(mspe_pre_br_Nivel_ApenasValoresY <- cs_InvPriv_Nivel_ApenasValoresY %>% 
   grab_signficance() %>% 
   filter(unit_name == "BR") %>% 
   pull(pre_mspe))
...
lista_graficos_ApenasValoresY <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPriv_Nivel_ApenasValoresY, 
   nome_variavel_y = "Investimento Privado Real PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
...
funcs$tabela_media_cs(cs_InvPriv_Nivel_ApenasValoresY, stargazer = F)
...
cs_InvPriv_Nivel_ApenasValoresY %>% grab_signficance()


### VENCEDOR: ÚLTIMO QUARTO VALORES Y (7.519) ####
## Tabela de médias
funcs$tabela_media_cs(cs_InvPriv_Nivel_UltQuartoValoresY, stargazer = T)

## Pesos e médias
lista_graficos_UltQuartoValoresY$pesos
ggsave("Figuras/Investimento_Privado/Pesos_InvPriv.pdf", dpi = 1200)
cs_InvPriv_Nivel_UltQuartoValoresY %>% 
   grab_unit_weights() %>% 
   arrange(desc(weight))

## Trends e Gaps
lista_graficos_UltQuartoValoresY$trends
ggsave("Figuras/Investimento_Privado/Trends_InvPriv.pdf", dpi = 1200)

lista_graficos_UltQuartoValoresY$gap
ggsave("Figuras/Investimento_Privado/Gap_InvPriv.pdf", dpi = 1200)

## Inferência
lista_graficos_UltQuartoValoresY$placebos
ggsave("Figuras/Investimento_Privado/Placebos_InvPriv.pdf", dpi = 1200)

lista_graficos_UltQuartoValoresY$razao_mspe
ggsave("Figuras/Investimento_Privado/Razao_MSPE_InvPriv.pdf", dpi = 1200)


## scpi ------------------------------------------------------------------------
### Normal #####################################################################
# Preparando dados
dfNivel_InvPriv_scpi <- scdata(
   dfNivel %>% filter(Ano %in% c(nAnoInicial:nAnoFinalInv)), 
   id.var = "iso2c", time.var = "Ano",
   outcome.var = "Investimento_Privado_PIB_PPP", 
...
cs_InvPriv_Nivel_scpi <- scest(dfNivel_InvPriv_scpi, 
                              w.constr = list(name = "simplex", Q = 1))

# Vendo resultados
summary(cs_InvPriv_Nivel_scpi)
scplot(cs_InvPriv_Nivel_scpi)
...
df_placebos_scpi <- funcs$placebos_scpi(
   ...
   string_variavel_interesse = "Investimento_Privado_PIB_PPP",
...
(mspe_pre_br_scpi <- funcs$mspe_pre_trat_scpi(cs_InvPriv_Nivel_scpi))
...
lista_graficos_scpi <- funcs$graficos_sc_scpi(
   objeto_sc_scpi = cs_InvPriv_Nivel_scpi,
...
# Estimando
cs_InvPriv_Nivel_scpi_ridge <- scest(dfNivel_InvPriv_scpi,
                                     w.constr = list(name = "ridge", Q = 1))

# Vendo resultados
summary(cs_InvPriv_Nivel_scpi_ridge)
scplot(cs_InvPriv_Nivel_scpi_ridge)
...
## MSPE do Brasil pré-tratamento
(mspe_pre_br_scpi_ridge <- funcs$mspe_pre_trat_scpi(cs_InvPriv_Nivel_scpi_ridge))
...
lista_graficos_scpi_ridge <- funcs$graficos_sc_scpi(
   objeto_sc_scpi = cs_InvPriv_Nivel_scpi_ridge,
...
cs_InvPriv_Nivel_scpi_lasso <- scest(dfNivel_InvPriv_scpi, 
                                    w.constr = list(name = "lasso", Q = 1))

# Vendo resultados
summary(cs_InvPriv_Nivel_scpi_lasso)
scplot(cs_InvPriv_Nivel_scpi_lasso)

# Placebos
df_placebos_scpi_lasso <- funcs$placebos_scpi(
   dados = dfNivel %>% filter(Ano %in% c(nAnoInicial:nAnoFinalInv)),
   string_coluna_unidade = "iso2c",
   string_coluna_tempo = "Ano",
   string_variavel_interesse = "Investimento_Privado_PIB_PPP",
   ano_tratamento = nAnoTratamento, unidade_tratada = "BR",
   ano_inicial = nAnoInicial, ano_final = nAnoFinalInv,
   restricao_w = list(name = "lasso", Q = 1)
)

## MSPE do Brasil pré-tratamento
(mspe_pre_br_scpi_lasso <- funcs$mspe_pre_trat_scpi(cs_InvPriv_Nivel_scpi_lasso))

# Razão MSPE Pós-Tratamento
df_razao_mspe_scpi_lasso <- funcs$razao_mspe_scpi(df_placebos_scpi_lasso)
mspe_pos_scpi_lasso <- mean(df_razao_mspe_scpi_lasso$post_mspe[
   df_razao_mspe_scpi_lasso$type == "donor"])

# Gráficos
lista_graficos_scpi_lasso <- funcs$graficos_sc_scpi(
   objeto_sc_scpi = cs_InvPriv_Nivel_scpi_lasso,
   df_placebos = df_placebos_scpi_lasso,
   df_razao = df_razao_mspe_scpi_lasso,
   nome_variavel_y = "Investimento Privado Real PPP (% do PIB)",
   ano_tratamento = nAnoTratamento, ano_inicial = nAnoInicial,
   ano_final = nAnoFinal, breaks_grafico = 2,
   paleta_trend = paleta_trend, paleta_gap = paleta_gap, 
...
lista_graficos_scpi_lasso$pesos

lista_graficos_scpi_lasso$trends

lista_graficos_scpi_lasso$gap

lista_graficos_scpi_lasso$placebos

lista_graficos_scpi_lasso$razao_mspe

### Demeaned - Ferman & Pinto (2021) ###########################################
# Preparando dados
dfNivel_InvPriv_scpi_demeaned <- scdata(
   dfNivel %>% filter(Ano %in% c(nAnoInicial:nAnoFinalInv)), 
   id.var = "iso2c", time.var = "Ano",
   outcome.var = "Investimento_Privado_PIB_PPP", 
   period.pre = (2005:2016),
...
# Estimando
cs_InvPriv_Nivel_scpi_demeaned <- scest(dfNivel_InvPriv_scpi_demeaned, 
                                       w.constr = list(name = "simplex", Q = 1))

# Vendo resultados
summary(cs_InvPriv_Nivel_scpi_demeaned)
scplot(cs_InvPriv_Nivel_scpi_demeaned)
...
## MSPE do Brasil pré-tratamento
(mspe_pre_br_scpi_demeaned <- funcs$mspe_pre_trat_scpi(cs_InvPriv_Nivel_scpi_demeaned))
...
lista_graficos_scpi_demeaned <- funcs$graficos_sc_scpi(
   objeto_sc_scpi = cs_InvPriv_Nivel_scpi_demeaned,
...
ggsave("Figuras/Investimento_Privado/Pesos_InvPriv_demeaned.pdf", dpi = 1200)

lista_graficos_scpi_demeaned$trends
ggsave("Figuras/Investimento_Privado/Trends_InvPriv_demeaned.pdf", dpi = 1200)

lista_graficos_scpi_demeaned$gap
ggsave("Figuras/Investimento_Privado/Gap_InvPriv_demeaned.pdf", dpi = 1200)

lista_graficos_scpi_demeaned$placebos
ggsave("Figuras/Investimento_Privado/Placebo_InvPriv_demeaned.pdf", dpi = 1200)

lista_graficos_scpi_demeaned$razao_mspe
ggsave("Figuras/Investimento_Privado/Razao_MSPE_InvPriv_demeaned.pdf", dpi = 1200)
