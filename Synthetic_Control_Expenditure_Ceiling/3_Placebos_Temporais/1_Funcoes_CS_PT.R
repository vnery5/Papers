...
graficos_sc_scpi <- function(objeto_sc_scpi, df_placebos, df_razao, 
   ...
   grafGap <- dfPlot %>% 
      ...
      ggplot(aes(x = time_unit, y = value, colour = name, fill = name)) +
      geom_line(size = 1.25) + 
      geom_ribbon(aes(ymin = 0, ymax = value), linetype=0, alpha=0.5) +
      geom_hline(yintercept = 0, linetype = 'dotted', 
                 size = .5, colour = 'black', show.legend = F) + 
      geom_textvline(xintercept = ano_tratamento, linetype = 'dotted', 
                     colour = 'black', size = 3,
...
# Placebos Temporais ==========================================================
graficos_cs_temporal <- function(objeto_cs_tidy, nome_variavel_y, 
                                 ano_placebo_temporal, ano_crise, ano_tratamento,
                                 ano_inicial, ano_final, breaks_grafico,
                                 escala_trends, escala_gaps,
                                 paleta_trend, paleta_gap, paleta_pesos,
                                 paleta_placebos, limite_mspe_placebos = 10,
   ...
   grafTrends <- dfTrends %>% 
      ...
      geom_line(size = 1.25) + 
      geom_point(size = 1.75) + 
      geom_textvline(xintercept = ano_placebo_temporal, linetype = 'dotted', 
                     colour = 'black', size = 3,
                     label = "Placebo Temporal", hjust = hjust_trends, vjust = .5) + 
      # geom_textvline(xintercept = ano_crise, linetype = 'dotted', 
      #                colour = 'black', size = 3,
      #                label = "Crise", hjust = .15, vjust = .5) + 
      geom_textvline(xintercept = ano_tratamento, linetype = 'dotted', 
                     colour = 'black', size = 3,
                     label = "Teto", hjust = hjust_trends, vjust = .5) + 
      labs(x = 'Ano', y = nome_variavel_y) +
      scale_x_continuous(breaks = seq(ano_inicial, ano_final, by = breaks_grafico)) +
      scale_y_continuous(limits = escala_trends[1:2]) +
      scale_colour_manual(name = "", values = paleta_trend, 
   ...
   grafGap <- dfGaps %>% 
      ...
      geom_textvline(xintercept = ano_placebo_temporal, linetype = 'dotted', 
                     colour = 'black', size = 3,
                     label = "Placebo Temporal", hjust = hjust_gap, vjust = .5) + 
      # geom_textvline(xintercept = ano_crise, linetype = 'dotted', 
      #                colour = 'black', size = 3,
      #                label = "Crise", hjust = .15, vjust = .5) + 
      geom_textvline(xintercept = ano_tratamento, linetype = 'dotted', 
                     colour = 'black', size = 3,
                     label = "Teto", hjust = hjust_gap, vjust = .5) + 
      labs(x = 'Ano', y = "Brasil - Sintético") +
      scale_x_continuous(breaks = seq(ano_inicial, ano_final, by = breaks_grafico)) +
      scale_y_continuous(limits = escala_gaps[1:2]) +
      scale_colour_manual(name = "", values = paleta_gap[1],
...
graficos_sc_scpi_temporal <- function(objeto_sc_scpi, df_placebos, df_razao, 
                                      nome_variavel_y, ano_tratamento, ano_placebo,
                                      ano_inicial, ano_final, breaks_grafico,
                                      paleta_trend, paleta_gap, paleta_pesos,
                                      paleta_placebos, limite_mspe_placebos = 10,
                                      hjust_trends = .85, hjust_gap = .85) {
   ...
   grafTrends <- dfPlot %>% 
      ...
      geom_line(size = 1.25) + 
      geom_point(size = 1.75) + 
      geom_textvline(xintercept = ano_placebo, linetype = 'dotted', 
                     colour = 'black', size = 3,
                     label = "Placebo Temporal", hjust = hjust_trends, vjust = .5) + 
      geom_textvline(xintercept = ano_tratamento, linetype = 'dotted', 
                     colour = 'black', size = 3,
