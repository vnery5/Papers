...
cs_Juro_Nom_NivelMediaY <- dfNorm %>% 
   ...
   generate_predictor(time_window = nAnoInicial:(nAnoInflexao - 1),
                      `Inflação 1` = mean(Inflacao_Consumidor, na.rm = T),
   ...
   generate_predictor(time_window = nAnoInflexao:nAnoTratamento,
                      `Inflação 2` = mean(Inflacao_Consumidor, na.rm = T),
...
#### México ####
# Preparando dados
dfNorm_Juro_Nom_scpi_MX <- scdata(
   dfNorm %>% filter(Ano %in% c(nAnoInicial:nAnoFinal)), 
   id.var = "iso2c", time.var = "Ano",
   outcome.var = "Taxa_Juros_Politica_CP", 
   period.pre = (2005:2016),
...
# Estimando
cs_Juro_Nom_scpi_MX <- scest(dfNorm_Juro_Nom_scpi_MX, 
                             w.constr = list(name = "simplex", Q = 1))


# Placebos
df_placebos_scpi_MX <- funcs$placebos_scpi(
   dados = dfNorm %>% filter(Ano %in% c(nAnoInicial:nAnoFinal)),
   string_coluna_unidade = "iso2c",
   string_coluna_tempo = "Ano",
   string_variavel_interesse = "Taxa_Juros_Politica_CP",
...
# Razão MSPE Pós-Tratamento
df_razao_mspe_scpi_MX <- funcs$razao_mspe_scpi(df_placebos_scpi_MX)
...
## Função
lista_graficos_scpi_MX <- funcs$graficos_sc_scpi_outros_paises(
   objeto_sc_scpi = cs_Juro_Nom_scpi_MX,
   df_placebos = df_placebos_scpi_MX,
   nome_pais = "México",
   nome_variavel_y = "Taxa de Juros Nominal (Média Anual, %)",
   ano_tratamento = nAnoTratamento, ano_inicial = nAnoInicial,
   ano_final = nAnoFinal, breaks_grafico = 2,
...
lista_graficos_scpi_MX$pesos
ggsave("Figuras/Taxa_Juros/Nom_Norm_Pesos_MX.pdf", dpi = 1200)

lista_graficos_scpi_MX$trends
ggsave("Figuras/Taxa_Juros/Nom_Norm_Trends_MX.pdf", dpi = 1200)

lista_graficos_scpi_MX$gap
ggsave("Figuras/Taxa_Juros/Nom_Norm_Gap_MX.pdf", dpi = 1200)

#### Malásia ####
# Preparando dados
dfNorm_Juro_Nom_scpi_MY <- scdata(
   dfNorm %>% filter(Ano %in% c(nAnoInicial:nAnoFinal)), 
   id.var = "iso2c", time.var = "Ano",
   outcome.var = "Taxa_Juros_Politica_CP", 
   period.pre = (2005:2016),
...
# Estimando
cs_Juro_Nom_scpi_MY <- scest(dfNorm_Juro_Nom_scpi_MY, 
                             w.constr = list(name = "simplex", Q = 1))

summary(cs_Juro_Nom_scpi_MY)

# Placebos
df_placebos_scpi_MY <- funcs$placebos_scpi(
   dados = dfNorm %>% filter(Ano %in% c(nAnoInicial:nAnoFinal)),
   string_coluna_unidade = "iso2c",
   string_coluna_tempo = "Ano",
   string_variavel_interesse = "Taxa_Juros_Politica_CP",
...
# Razão MSPE Pós-Tratamento
df_razao_mspe_scpi_MY <- funcs$razao_mspe_scpi(df_placebos_scpi_MY)
...
## Função
lista_graficos_scpi_MY <- funcs$graficos_sc_scpi_outros_paises(
   objeto_sc_scpi = cs_Juro_Nom_scpi_MY,
   df_placebos = df_placebos_scpi_MY,
   nome_pais = "Malásia",
   nome_variavel_y = "Taxa de Juros Nominal (Média Anual, %)",
   ano_tratamento = nAnoTratamento, ano_inicial = nAnoInicial,
   ano_final = nAnoFinal, breaks_grafico = 2,
...
lista_graficos_scpi_MY$pesos
ggsave("Figuras/Taxa_Juros/Nom_Norm_Pesos_MY.pdf", dpi = 1200)

lista_graficos_scpi_MY$trends
ggsave("Figuras/Taxa_Juros/Nom_Norm_Trends_MY.pdf", dpi = 1200)

lista_graficos_scpi_MY$gap
ggsave("Figuras/Taxa_Juros/Nom_Norm_Gap_MY.pdf", dpi = 1200)
