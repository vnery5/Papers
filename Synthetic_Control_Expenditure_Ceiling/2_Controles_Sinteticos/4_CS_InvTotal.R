# Setup e Bibliotecas ==========================================================
# Dados
library(tidyverse)
library(magrittr)
library(lubridate)
library(readxl)
library(skimr)

# Controle Sintético
library(tidysynth)
library(scpi)

# Gráficos e Tabelas
library(wesanderson)  # https://github.com/karthik/wesanderson
library(MetBrewer)  # https://github.com/BlakeRMills/MetBrewer/tree/main
library(stargazer)
library(ggplot2)
library(geomtextpath)

# Comandos Úteis
rm(list = ls())
cat("\014")

# Settando o diretório para o do arquivo
setwd(dirname(rstudioapi::getActiveDocumentContext()$path))
sDiretorio <- dirname(rstudioapi::getActiveDocumentContext()$path)

# Baixando funções
funcs <- new.env()
source("1_Funcoes_CS.R", local = funcs, encoding = 'UTF-8')

# Customizando paletas e escalas
paleta_trend <- met.brewer("Signac")[c(14, 6)]
paleta_gap <- met.brewer("Benedictus")[c(13, 11)]
paleta_pesos <- c(met.brewer("Java")[3], met.brewer("Johnson")[4])
paleta_placebos <- c("lightgray", met.brewer("Signac")[14])

breaks_grafico <- 2
escala_trends <- c(1, 6, 1)
escala_gaps <- c(-1.5, 1, 0.25)
escala_trends <- NULL
escala_gaps <- NULL

# Definindo anos iniciais, pré-tratamento e finais
nAnoInicial <- 2005
nAnoInflexao <- 2012
nAnoTratamento <- 2016
nAnoFinalInv <- 2019
nAnoFinal <- 2021

# Dados ========================================================================
# Baixando
dfTodos <- read_excel(paste(sDiretorio, 
                            "Base_Investimento_Deficit_Juros.xlsx", 
                            sep = "/"),
                      sheet = "Dataset_Long")

# Definindo conjunto de doadores + Brasil
vDoadores <- c("BR",
               "ZA", "CL", "CO", "HU", "MX", "MY", "PE", "PL", "RU", "TH", "TR",
               "AR", "CN", "EC", "IN", "UY", "BG")

vDoadores_Juros <- c("BR",
                     "ZA", "CL", "CO", "HU", "MX", "MY", 
                     "PE", "PL", "RU", "TH", "TR")

# Lista de variáveis
cCovariadas <- c("Resultado_Primario_CA_PIB_Pot", "Inflacao_Consumidor",
                 "PIB_Capita_PPP17", "Taxa_Desemprego", "Valor_Add_Industria",
                 "Termos_Troca", "Conta_Corrente_PIB", "Taxa_Cambio100",
                 "Controle_Corrupcao", "Estabilidade_Politica")

cInv <- c("Investimento_Privado_PIB_PPP", 
          "Investimento_Publico_PIB_PPP",
          "Investimento_PIB_PPP", 
          "Investimento_PIB")

cJuros <- c("Taxa_Juros_Politica_CP", "Taxa_Juros_Real_Politica_CP")

cY <- c(cInv, cJuros)

# Restringindo e criando uma coluna de tratado
df <- dfTodos %>% 
   filter(iso2c %in% vDoadores) %>% 
   filter(Variável %in% c(cCovariadas, cY)) %>% 
   filter(Ano >= nAnoInicial) %>% 
   group_by(iso2c, Variável) %>% 
   arrange(Ano) %>% 
   mutate(PD = ifelse(Variável %in% cY, Valor - lag(Valor), Valor),
          Delta = ifelse(Variável %in% cY, Valor / lag(Valor), Valor),
          Valor100 = ifelse(Variável %in% cY, cumprod(c(100, Delta[-1])), Valor)) %>%  # 2016 = 100
   select(-Delta) %>% 
   as.data.frame() %>% 
   mutate(Tratado = ifelse((iso2c == "BR") & (Ano > nAnoTratamento), 1, 0))

## Datasets Largos -------------------------------------------------------------
# Nível
dfNivel <- df %>% 
   select(-PD, -Valor100) %>% 
   pivot_wider(names_from = Variável, values_from = Valor)

# Primeiras diferenças
dfPD <- df %>% 
   select(-Valor, -Valor100) %>% 
   pivot_wider(names_from = Variável, values_from = PD)

# 2003 = 100
dfNorm <- df %>% 
   select(-PD, -Valor) %>% 
   pivot_wider(names_from = Variável, values_from = Valor100)

# # Fazendo 2016 = 100
# ## DataFrame auxiliar
# df2016 <- dfNorm %>%
#     filter(Ano == 2016) %>%
#     select(iso2c, Variável, Valor100) %>%
#     rename(Valor100_2016 = Valor100)
# 
# ## Joining
# dfNorm %<>%
#     left_join(df2016, by = c("iso2c", "Variável")) %>%
#     mutate(Valor100 = ifelse(Variável %in% cY, Valor100 * 100 / Valor100_2016, Valor100)) %>%
#     select(-Valor100_2016) %>% 
#     pivot_wider(names_from = Variável, values_from = Valor100)

# Controles Sintéticos =========================================================
## Nível -----------------------------------------------------------------------
### 1. Média de Y ####
# Controle Sintético
cs_InvTot_NivelMediaY <- dfNivel %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
                     i_unit = "BR",
                     i_time = nAnoTratamento,
                     generate_placebos = TRUE) %>% 
   # Gerando preditores: médias
   ## Até 2011
   generate_predictor(time_window = nAnoInicial:(nAnoInflexao - 1),
                      `Taxa de Cambio 100 1` = mean(Taxa_Cambio100, na.rm = T),
                      `PIB per Capita PPP 1` = mean(PIB_Capita_PPP17, na.rm = T),
                      `Termos de Troca 1` = mean(Termos_Troca, na.rm = T),
                      `Resultado Primário CA 1` = mean(Resultado_Primario_CA_PIB_Pot, na.rm = T),
                      `Controle Corrupção 1` = mean(Controle_Corrupcao, na.rm = T),
                      `Taxa Desemprego 1` = mean(Taxa_Desemprego, na.rm = T)
   ) %>% 
   ## 2012 a 2016
   generate_predictor(time_window = nAnoInflexao:nAnoTratamento,
                      `Taxa de Cambio 100 2` = mean(Taxa_Cambio100, na.rm = T),
                      `PIB per Capita PPP 2` = mean(PIB_Capita_PPP17, na.rm = T),
                      `Termos de Troca 2` = mean(Termos_Troca, na.rm = T),
                      `Resultado Primário CA 2` = mean(Resultado_Primario_CA_PIB_Pot, na.rm = T),
                      `Controle Corrupção 2` = mean(Controle_Corrupcao, na.rm = T),
                      `Taxa Desemprego 2` = mean(Taxa_Desemprego, na.rm = T)
   ) %>% 
   # Gerando preditores: valores pré_tratamento
   generate_predictor(time_window = nAnoInicial:nAnoTratamento,
                      `Investimento` = mean(Investimento_PIB_PPP, na.rm = T)) %>% 
   generate_weights(optimization_window = nAnoInicial:nAnoTratamento) %>% 
   generate_control()

# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_NivelMediaY <- funcs$mspe_placebos_pos_trat(cs_InvTot_NivelMediaY,
                                                         nAnoTratamento))
(mspe_pre_br_NivelMediaY <- cs_InvTot_NivelMediaY %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))

# Gráficos
lista_graficosMediaY <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvTot_NivelMediaY, 
   nome_variavel_y = "Investimento Real PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
   breaks_grafico = breaks_grafico,
   escala_trends = escala_trends,
   escala_gaps = escala_gaps,
   paleta_trend = paleta_trend, 
   paleta_gap = paleta_gap, 
   paleta_pesos = paleta_pesos,
   paleta_placebos = paleta_placebos, 
   limite_mspe_placebos = 10
)

## Pesos e médias
lista_graficosMediaY$pesos
funcs$tabela_media_cs(cs_InvTot_NivelMediaY, stargazer = F)

## Trends e Gaps
lista_graficosMediaY$trends
lista_graficosMediaY$gap

## Inferência
lista_graficosMediaY$placebos
lista_graficosMediaY$razao_mspe
cs_InvTot_NivelMediaY %>% grab_signficance()

### 2. Todos os Valores de Y ####
# Controle Sintético
cs_InvTot_Nivel_ValoresY <- dfNivel %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
                     i_unit = "BR",
                     i_time = nAnoTratamento,
                     generate_placebos = TRUE) %>% 
   # Gerando preditores: médias
   ## Até 2011
   generate_predictor(time_window = nAnoInicial:(nAnoInflexao - 1),
                      `Taxa de Cambio 100 1` = mean(Taxa_Cambio100, na.rm = T),
                      `PIB per Capita PPP 1` = mean(PIB_Capita_PPP17, na.rm = T),
                      `Termos de Troca 1` = mean(Termos_Troca, na.rm = T),
                      `Resultado Primário CA 1` = mean(Resultado_Primario_CA_PIB_Pot, na.rm = T),
                      `Controle Corrupção 1` = mean(Controle_Corrupcao, na.rm = T),
                      `Taxa Desemprego 1` = mean(Taxa_Desemprego, na.rm = T)
   ) %>% 
   ## 2012 a 2016
   generate_predictor(time_window = nAnoInflexao:nAnoTratamento,
                      `Taxa de Cambio 100 2` = mean(Taxa_Cambio100, na.rm = T),
                      `PIB per Capita PPP 2` = mean(PIB_Capita_PPP17, na.rm = T),
                      `Termos de Troca 2` = mean(Termos_Troca, na.rm = T),
                      `Resultado Primário CA 2` = mean(Resultado_Primario_CA_PIB_Pot, na.rm = T),
                      `Controle Corrupção 2` = mean(Controle_Corrupcao, na.rm = T),
                      `Taxa Desemprego 2` = mean(Taxa_Desemprego, na.rm = T)
   ) %>% 
   # Gerando preditores: valores pré_tratamento
   generate_predictor(time_window = 2005,
                      `Investimento 2005` = Investimento_PIB_PPP) %>%
   generate_predictor(time_window = 2006,
                      `Investimento 2006` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2007,
                      `Investimento 2007` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2008,
                      `Investimento 2008` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2009,
                      `Investimento 2009` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2010,
                      `Investimento 2010` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2011,
                      `Investimento 2011` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2012,
                      `Investimento 2012` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2013,
                      `Investimento 2013` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2014,
                      `Investimento 2014` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2015,
                      `Investimento 2015` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2016,
                      `Investimento 2016` = Investimento_PIB_PPP) %>% 
   generate_weights(optimization_window = nAnoInicial:nAnoTratamento) %>% 
   generate_control()

# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_Nivel_ValoresY <- funcs$mspe_placebos_pos_trat(cs_InvTot_Nivel_ValoresY,
                                                         nAnoTratamento))
(mspe_pre_br_Nivel_ValoresY <- cs_InvTot_Nivel_ValoresY %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))

# Gráficos
lista_graficos_ValoresY <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvTot_Nivel_ValoresY, 
   nome_variavel_y = "Investimento Real PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
   breaks_grafico = breaks_grafico,
   escala_trends = escala_trends,
   escala_gaps = escala_gaps,
   paleta_trend = paleta_trend, 
   paleta_gap = paleta_gap, 
   paleta_pesos = paleta_pesos,
   paleta_placebos = paleta_placebos, 
   limite_mspe_placebos = 10
)

## Pesos e médias
lista_graficos_ValoresY$pesos
funcs$tabela_media_cs(cs_InvTot_Nivel_ValoresY, stargazer = F)

## Trends e Gaps
lista_graficos_ValoresY$trends
lista_graficos_ValoresY$gap

## Inferência
lista_graficos_ValoresY$placebos
lista_graficos_ValoresY$razao_mspe
cs_InvTot_Nivel_ValoresY %>% grab_signficance()

### 3. Inflexões ####
# Controle Sintético
cs_InvTot_Nivel_Inflexoes <- dfNivel %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
                     i_unit = "BR",
                     i_time = nAnoTratamento,
                     generate_placebos = TRUE) %>% 
   # Gerando preditores: médias
   ## Até 2011
   generate_predictor(time_window = nAnoInicial:(nAnoInflexao - 1),
                      `Taxa de Cambio 100 1` = mean(Taxa_Cambio100, na.rm = T),
                      `PIB per Capita PPP 1` = mean(PIB_Capita_PPP17, na.rm = T),
                      `Termos de Troca 1` = mean(Termos_Troca, na.rm = T),
                      `Resultado Primário CA 1` = mean(Resultado_Primario_CA_PIB_Pot, na.rm = T),
                      `Controle Corrupção 1` = mean(Controle_Corrupcao, na.rm = T),
                      `Taxa Desemprego 1` = mean(Taxa_Desemprego, na.rm = T)
   ) %>% 
   ## 2012 a 2016
   generate_predictor(time_window = nAnoInflexao:nAnoTratamento,
                      `Taxa de Cambio 100 2` = mean(Taxa_Cambio100, na.rm = T),
                      `PIB per Capita PPP 2` = mean(PIB_Capita_PPP17, na.rm = T),
                      `Termos de Troca 2` = mean(Termos_Troca, na.rm = T),
                      `Resultado Primário CA 2` = mean(Resultado_Primario_CA_PIB_Pot, na.rm = T),
                      `Controle Corrupção 2` = mean(Controle_Corrupcao, na.rm = T),
                      `Taxa Desemprego 2` = mean(Taxa_Desemprego, na.rm = T)
   ) %>% 
   generate_predictor(time_window = nAnoInicial,
                      `Investimento 2005` = Investimento_PIB_PPP) %>%
   generate_predictor(time_window = nAnoInflexao - 1,
                      `Investimento 2011` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = nAnoTratamento,
                      `Investimento 2016` = Investimento_PIB_PPP) %>% 
   generate_weights(optimization_window = nAnoInicial:nAnoTratamento) %>% 
   generate_control()

# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_Nivel_Inflexoes <- funcs$mspe_placebos_pos_trat(cs_InvTot_Nivel_Inflexoes, 
                                                          nAnoTratamento))
(mspe_pre_br_Nivel_Inflexoes <- cs_InvTot_Nivel_Inflexoes %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))

# Gráficos
lista_graficos_Inflexoes <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvTot_Nivel_Inflexoes, 
   nome_variavel_y = "Investimento Real PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
   breaks_grafico = breaks_grafico,
   escala_trends = escala_trends,
   escala_gaps = escala_gaps,
   paleta_trend = paleta_trend, 
   paleta_gap = paleta_gap, 
   paleta_pesos = paleta_pesos,
   paleta_placebos = paleta_placebos, 
   limite_mspe_placebos = 10
)

## Pesos e médias
lista_graficos_Inflexoes$pesos
funcs$tabela_media_cs(cs_InvTot_Nivel_Inflexoes, stargazer = F)

## Trends e Gaps
lista_graficos_Inflexoes$trends
lista_graficos_Inflexoes$gap

## Inferência
lista_graficos_Inflexoes$placebos
lista_graficos_Inflexoes$razao_mspe
cs_InvTot_Nivel_Inflexoes %>% grab_signficance()


### 4. Primeiros 75% dos Valores de Y ####
cs_InvTot_Nivel_TresQuartosValoresY <- dfNivel %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
                     i_unit = "BR",
                     i_time = nAnoTratamento,
                     generate_placebos = TRUE) %>% 
   # Gerando preditores: médias
   ## Até 2011
   generate_predictor(time_window = nAnoInicial:(nAnoInflexao - 1),
                      `Taxa de Cambio 100 1` = mean(Taxa_Cambio100, na.rm = T),
                      `PIB per Capita PPP 1` = mean(PIB_Capita_PPP17, na.rm = T),
                      `Termos de Troca 1` = mean(Termos_Troca, na.rm = T),
                      `Resultado Primário CA 1` = mean(Resultado_Primario_CA_PIB_Pot, na.rm = T),
                      `Controle Corrupção 1` = mean(Controle_Corrupcao, na.rm = T),
                      `Taxa Desemprego 1` = mean(Taxa_Desemprego, na.rm = T)
   ) %>% 
   ## 2012 a 2016
   generate_predictor(time_window = nAnoInflexao:nAnoTratamento,
                      `Taxa de Cambio 100 2` = mean(Taxa_Cambio100, na.rm = T),
                      `PIB per Capita PPP 2` = mean(PIB_Capita_PPP17, na.rm = T),
                      `Termos de Troca 2` = mean(Termos_Troca, na.rm = T),
                      `Resultado Primário CA 2` = mean(Resultado_Primario_CA_PIB_Pot, na.rm = T),
                      `Controle Corrupção 2` = mean(Controle_Corrupcao, na.rm = T),
                      `Taxa Desemprego 2` = mean(Taxa_Desemprego, na.rm = T)
   ) %>% 
   # Gerando preditores: valores pré_tratamento
   generate_predictor(time_window = 2005,
                      `Investimento 2005` = Investimento_PIB_PPP) %>%
   generate_predictor(time_window = 2006,
                      `Investimento 2006` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2007,
                      `Investimento 2007` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2008,
                      `Investimento 2008` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2009,
                      `Investimento 2009` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2010,
                      `Investimento 2010` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2011,
                      `Investimento 2011` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2012,
                      `Investimento 2012` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2013,
                      `Investimento 2013` = Investimento_PIB_PPP) %>% 
   generate_weights(optimization_window = nAnoInicial:nAnoTratamento) %>% 
   generate_control()

# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_Nivel_TresQuartosValoresY <- funcs$mspe_placebos_pos_trat(
   cs_InvTot_Nivel_TresQuartosValoresY, nAnoTratamento
))
(mspe_pre_br_Nivel_TresQuartosValoresY <- cs_InvTot_Nivel_TresQuartosValoresY %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))

# Gráficos
lista_graficos_TresQuartosValoresY <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvTot_Nivel_TresQuartosValoresY, 
   nome_variavel_y = "Investimento Real PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
   breaks_grafico = breaks_grafico,
   escala_trends = escala_trends,
   escala_gaps = escala_gaps,
   paleta_trend = paleta_trend, 
   paleta_gap = paleta_gap, 
   paleta_pesos = paleta_pesos,
   paleta_placebos = paleta_placebos, 
   limite_mspe_placebos = 10
)

## Pesos e médias
lista_graficos_TresQuartosValoresY$pesos
funcs$tabela_media_cs(cs_InvTot_Nivel_TresQuartosValoresY, stargazer = F)

## Trends e Gaps
lista_graficos_TresQuartosValoresY$trends
lista_graficos_TresQuartosValoresY$gap

## Inferência
lista_graficos_TresQuartosValoresY$placebos
lista_graficos_TresQuartosValoresY$razao_mspe
cs_InvTot_Nivel_TresQuartosValoresY %>% grab_signficance()

### 5. Últimos 25% dos Valores de Y ####
cs_InvTot_Nivel_UltQuartoValoresY <- dfNivel %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
                     i_unit = "BR",
                     i_time = nAnoTratamento,
                     generate_placebos = TRUE) %>% 
   # Gerando preditores: médias
   ## Até 2011
   generate_predictor(time_window = nAnoInicial:(nAnoInflexao - 1),
                      `Taxa de Cambio 100 1` = mean(Taxa_Cambio100, na.rm = T),
                      `PIB per Capita PPP 1` = mean(PIB_Capita_PPP17, na.rm = T),
                      `Termos de Troca 1` = mean(Termos_Troca, na.rm = T),
                      `Resultado Primário CA 1` = mean(Resultado_Primario_CA_PIB_Pot, na.rm = T),
                      `Controle Corrupção 1` = mean(Controle_Corrupcao, na.rm = T),
                      `Taxa Desemprego 1` = mean(Taxa_Desemprego, na.rm = T)
   ) %>% 
   ## 2012 a 2016
   generate_predictor(time_window = nAnoInflexao:nAnoTratamento,
                      `Taxa de Cambio 100 2` = mean(Taxa_Cambio100, na.rm = T),
                      `PIB per Capita PPP 2` = mean(PIB_Capita_PPP17, na.rm = T),
                      `Termos de Troca 2` = mean(Termos_Troca, na.rm = T),
                      `Resultado Primário CA 2` = mean(Resultado_Primario_CA_PIB_Pot, na.rm = T),
                      `Controle Corrupção 2` = mean(Controle_Corrupcao, na.rm = T),
                      `Taxa Desemprego 2` = mean(Taxa_Desemprego, na.rm = T)
   ) %>% 
   # Gerando preditores: valores pré_tratamento
   generate_predictor(time_window = 2014,
                      `Investimento 2014` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2015,
                      `Investimento 2015` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2016,
                      `Investimento 2016` = Investimento_PIB_PPP) %>% 
   generate_weights(optimization_window = nAnoInicial:nAnoTratamento) %>% 
   generate_control()

# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_Nivel_UltQuartoValoresY <- funcs$mspe_placebos_pos_trat(
   cs_InvTot_Nivel_UltQuartoValoresY, nAnoTratamento
))
(mspe_pre_br_Nivel_UltQuartoValoresY <- cs_InvTot_Nivel_UltQuartoValoresY %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))

# Gráficos
lista_graficos_UltQuartoValoresY <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvTot_Nivel_UltQuartoValoresY, 
   nome_variavel_y = "Investimento Real PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
   breaks_grafico = breaks_grafico,
   escala_trends = escala_trends,
   escala_gaps = escala_gaps,
   paleta_trend = paleta_trend, 
   paleta_gap = paleta_gap, 
   paleta_pesos = paleta_pesos,
   paleta_placebos = paleta_placebos, 
   limite_mspe_placebos = 10
)

## Pesos e médias
lista_graficos_UltQuartoValoresY$pesos
funcs$tabela_media_cs(cs_InvTot_Nivel_UltQuartoValoresY, stargazer = F)

## Trends e Gaps
lista_graficos_UltQuartoValoresY$trends
lista_graficos_UltQuartoValoresY$gap

## Inferência
lista_graficos_UltQuartoValoresY$placebos
lista_graficos_UltQuartoValoresY$razao_mspe
cs_InvTot_Nivel_UltQuartoValoresY %>% grab_signficance()


### 6. Apenas Covariadas Externas ####
cs_InvTot_Nivel_Controles <- dfNivel %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
                     i_unit = "BR",
                     i_time = nAnoTratamento,
                     generate_placebos = TRUE) %>% 
   # Gerando preditores: médias
   ## Até 2011
   generate_predictor(time_window = nAnoInicial:(nAnoInflexao - 1),
                      `Taxa de Cambio 100 1` = mean(Taxa_Cambio100, na.rm = T),
                      `PIB per Capita PPP 1` = mean(PIB_Capita_PPP17, na.rm = T),
                      `Termos de Troca 1` = mean(Termos_Troca, na.rm = T),
                      `Resultado Primário CA 1` = mean(Resultado_Primario_CA_PIB_Pot, na.rm = T),
                      `Controle Corrupção 1` = mean(Controle_Corrupcao, na.rm = T),
                      `Taxa Desemprego 1` = mean(Taxa_Desemprego, na.rm = T)
   ) %>% 
   ## 2012 a 2016
   generate_predictor(time_window = nAnoInflexao:nAnoTratamento,
                      `Taxa de Cambio 100 2` = mean(Taxa_Cambio100, na.rm = T),
                      `PIB per Capita PPP 2` = mean(PIB_Capita_PPP17, na.rm = T),
                      `Termos de Troca 2` = mean(Termos_Troca, na.rm = T),
                      `Resultado Primário CA 2` = mean(Resultado_Primario_CA_PIB_Pot, na.rm = T),
                      `Controle Corrupção 2` = mean(Controle_Corrupcao, na.rm = T),
                      `Taxa Desemprego 2` = mean(Taxa_Desemprego, na.rm = T),
   ) %>% 
   generate_weights(optimization_window = nAnoInicial:nAnoTratamento) %>% 
   generate_control()

# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_Nivel_Controles <- funcs$mspe_placebos_pos_trat(cs_InvTot_Nivel_Controles, 
                                                          nAnoTratamento))
(mspe_pre_br_Nivel_Controles <- cs_InvTot_Nivel_Controles %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))

# Gráficos
lista_graficos_Controles <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvTot_Nivel_Controles, 
   nome_variavel_y = "Investimento Real PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
   breaks_grafico = breaks_grafico,
   escala_trends = escala_trends,
   escala_gaps = escala_gaps,
   paleta_trend = paleta_trend, 
   paleta_gap = paleta_gap, 
   paleta_pesos = paleta_pesos,
   paleta_placebos = paleta_placebos, 
   limite_mspe_placebos = 10
)

## Pesos e médias
lista_graficos_Controles$pesos
funcs$tabela_media_cs(cs_InvTot_Nivel_Controles, stargazer = F)

## Trends e Gaps
lista_graficos_Controles$trends
lista_graficos_Controles$gap

## Inferência
lista_graficos_Controles$placebos
lista_graficos_Controles$razao_mspe
cs_InvTot_Nivel_Controles %>% grab_signficance()


### 7. Apenas os Valores de Y ####
# Controle Sintético
cs_InvTot_Nivel_ApenasValoresY <- dfNivel %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
                     i_unit = "BR",
                     i_time = nAnoTratamento,
                     generate_placebos = TRUE) %>% 
   # Gerando preditores: valores pré_tratamento
   generate_predictor(time_window = 2005,
                      `Investimento 2005` = Investimento_PIB_PPP) %>%
   generate_predictor(time_window = 2006,
                      `Investimento 2006` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2007,
                      `Investimento 2007` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2008,
                      `Investimento 2008` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2009,
                      `Investimento 2009` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2010,
                      `Investimento 2010` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2011,
                      `Investimento 2011` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2012,
                      `Investimento 2012` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2013,
                      `Investimento 2013` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2014,
                      `Investimento 2014` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2015,
                      `Investimento 2015` = Investimento_PIB_PPP) %>% 
   generate_predictor(time_window = 2016,
                      `Investimento 2016` = Investimento_PIB_PPP) %>% 
   generate_weights(optimization_window = nAnoInicial:nAnoTratamento) %>% 
   generate_control()

# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_Nivel_ApenasValoresY <- funcs$mspe_placebos_pos_trat(
   cs_InvTot_Nivel_ApenasValoresY,nAnoTratamento
))

(mspe_pre_br_Nivel_ApenasValoresY <- cs_InvTot_Nivel_ApenasValoresY %>% 
   grab_signficance() %>% 
   filter(unit_name == "BR") %>% 
   pull(pre_mspe))

# Gráficos
lista_graficos_ApenasValoresY <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvTot_Nivel_ApenasValoresY, 
   nome_variavel_y = "Investimento Real PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
   breaks_grafico = breaks_grafico,
   escala_trends = escala_trends,
   escala_gaps = escala_gaps,
   paleta_trend = paleta_trend, 
   paleta_gap = paleta_gap, 
   paleta_pesos = paleta_pesos,
   paleta_placebos = paleta_placebos, 
   limite_mspe_placebos = 10,
   hjust_gap = .95
)

## Pesos e médias
lista_graficos_ApenasValoresY$pesos
funcs$tabela_media_cs(cs_InvTot_Nivel_ApenasValoresY, stargazer = F)

## Trends e Gaps
lista_graficos_ApenasValoresY$trends
lista_graficos_ApenasValoresY$gap

## Inferência
lista_graficos_ApenasValoresY$placebos
lista_graficos_ApenasValoresY$razao_mspe
cs_InvTot_Nivel_ApenasValoresY %>% grab_signficance()


### VENCEDOR: APENAS VALORES Y (28.28) ####
## Pesos e médias
lista_graficos_ApenasValoresY$pesos
ggsave("Figuras/Investimento_Total/Pesos_InvTot.pdf", dpi = 1200)

## Trends e Gaps
lista_graficos_ApenasValoresY$trends
ggsave("Figuras/Investimento_Total/Trends_InvTot.pdf", dpi = 1200)

lista_graficos_ApenasValoresY$gap
ggsave("Figuras/Investimento_Total/Gap_InvTot.pdf", dpi = 1200)

## Inferência
lista_graficos_ApenasValoresY$placebos
ggsave("Figuras/Investimento_Total/Placebos_InvTot.pdf", dpi = 1200)

lista_graficos_ApenasValoresY$razao_mspe
ggsave("Figuras/Investimento_Total/Razao_MSPE_InvTot.pdf", dpi = 1200)


## scpi ------------------------------------------------------------------------
### Normal #####################################################################
# Preparando dados
dfNivel_InvTot_scpi <- scdata(
   dfNivel %>% filter(Ano %in% c(nAnoInicial:nAnoFinalInv)), 
   id.var = "iso2c", time.var = "Ano",
   outcome.var = "Investimento_PIB_PPP", 
   period.pre = (2005:2016),
   period.post = (2017:2019), 
   unit.tr = "BR", unit.co = vDoadores[-1]
)

# Estimando com pesos estritamente positivos ou zero
cs_InvTot_Nivel_scpi <- scest(dfNivel_InvTot_scpi, 
                              w.constr = list(name = "simplex", Q = 1))

# Vendo resultados
summary(cs_InvTot_Nivel_scpi)
scplot(cs_InvTot_Nivel_scpi)

# Placebos
df_placebos_scpi <- funcs$placebos_scpi(
   dados = dfNivel %>% filter(Ano %in% c(nAnoInicial:nAnoFinalInv)),
   string_coluna_unidade = "iso2c",
   string_coluna_tempo = "Ano",
   string_variavel_interesse = "Investimento_PIB_PPP",
   ano_tratamento = nAnoTratamento, unidade_tratada = "BR",
   ano_inicial = nAnoInicial, ano_final = nAnoFinalInv,
   restricao_w = list(name = "simplex", Q = 1)
)

## MSPE do Brasil pré-tratamento
(mspe_pre_br_scpi <- funcs$mspe_pre_trat_scpi(cs_InvTot_Nivel_scpi))

# Razão MSPE Pós-Tratamento
df_razao_mspe_scpi <- funcs$razao_mspe_scpi(df_placebos_scpi)
mspe_pos_scpi <- mean(df_razao_mspe_scpi$post_mspe[df_razao_mspe_scpi$type == "donor"])

# Gráficos
lista_graficos_scpi <- funcs$graficos_sc_scpi(
   objeto_sc_scpi = cs_InvTot_Nivel_scpi,
   df_placebos = df_placebos_scpi,
   df_razao = df_razao_mspe_scpi,
   nome_variavel_y = "Investimento Real PPP (% do PIB)",
   ano_tratamento = nAnoTratamento, ano_inicial = nAnoInicial,
   ano_final = nAnoFinal, breaks_grafico = 2,
   paleta_trend = paleta_trend, paleta_gap = paleta_gap, 
   paleta_pesos = paleta_pesos, paleta_placebos = paleta_placebos
)

lista_graficos_scpi$pesos
lista_graficos_scpi$trends
lista_graficos_scpi$gap

lista_graficos_scpi$placebos
lista_graficos_scpi$razao_mspe

### Ridge - Amjad et al. (2018) ################################################
# Estimando
cs_InvTot_Nivel_scpi_ridge <- scest(dfNivel_InvTot_scpi,
                                     w.constr = list(name = "ridge", Q = 1))

# Vendo resultados
summary(cs_InvTot_Nivel_scpi_ridge)
scplot(cs_InvTot_Nivel_scpi_ridge)

# Placebos
df_placebos_scpi_ridge <- funcs$placebos_scpi(
   dados = dfNivel %>% filter(Ano %in% c(nAnoInicial:nAnoFinalInv)),
   string_coluna_unidade = "iso2c",
   string_coluna_tempo = "Ano",
   string_variavel_interesse = "Investimento_PIB_PPP",
   ano_tratamento = nAnoTratamento, unidade_tratada = "BR",
   ano_inicial = nAnoInicial, ano_final = nAnoFinalInv,
   restricao_w = list(name = "ridge", Q = 1)
)

## MSPE do Brasil pré-tratamento
(mspe_pre_br_scpi_ridge <- funcs$mspe_pre_trat_scpi(cs_InvTot_Nivel_scpi_ridge))

# Razão MSPE Pós-Tratamento
df_razao_mspe_scpi_ridge <- funcs$razao_mspe_scpi(df_placebos_scpi_ridge)
mspe_pos_scpi_ridge <- mean(df_razao_mspe_scpi_ridge$post_mspe[
   df_razao_mspe_scpi_ridge$type == "donor"])

# Gráficos
lista_graficos_scpi_ridge <- funcs$graficos_sc_scpi(
   objeto_sc_scpi = cs_InvTot_Nivel_scpi_ridge,
   df_placebos = df_placebos_scpi_ridge,
   df_razao = df_razao_mspe_scpi_ridge,
   nome_variavel_y = "Investimento Real PPP (% do PIB)",
   ano_tratamento = nAnoTratamento, ano_inicial = nAnoInicial,
   ano_final = nAnoFinal, breaks_grafico = 2,
   paleta_trend = paleta_trend, paleta_gap = paleta_gap, 
   paleta_pesos = paleta_pesos, paleta_placebos = paleta_placebos
)

lista_graficos_scpi_ridge$pesos
lista_graficos_scpi_ridge$trends
lista_graficos_scpi_ridge$gap

lista_graficos_scpi_ridge$placebos
lista_graficos_scpi_ridge$razao_mspe

### Lasso - Chernozhukov et al. (2021) #########################################
cs_InvTot_Nivel_scpi_lasso <- scest(dfNivel_InvTot_scpi, 
                                    w.constr = list(name = "lasso", Q = 1))

# Vendo resultados
summary(cs_InvTot_Nivel_scpi_lasso)
scplot(cs_InvTot_Nivel_scpi_lasso)

# Placebos
df_placebos_scpi_lasso <- funcs$placebos_scpi(
   dados = dfNivel %>% filter(Ano %in% c(nAnoInicial:nAnoFinalInv)),
   string_coluna_unidade = "iso2c",
   string_coluna_tempo = "Ano",
   string_variavel_interesse = "Investimento_PIB_PPP",
   ano_tratamento = nAnoTratamento, unidade_tratada = "BR",
   ano_inicial = nAnoInicial, ano_final = nAnoFinalInv,
   restricao_w = list(name = "lasso", Q = 1)
)

## MSPE do Brasil pré-tratamento
(mspe_pre_br_scpi_lasso <- funcs$mspe_pre_trat_scpi(cs_InvTot_Nivel_scpi_lasso))

# Razão MSPE Pós-Tratamento
df_razao_mspe_scpi_lasso <- funcs$razao_mspe_scpi(df_placebos_scpi_lasso)
mspe_pos_scpi_lasso <- mean(df_razao_mspe_scpi_lasso$post_mspe[
   df_razao_mspe_scpi_lasso$type == "donor"])

# Gráficos
lista_graficos_scpi_lasso <- funcs$graficos_sc_scpi(
   objeto_sc_scpi = cs_InvTot_Nivel_scpi_lasso,
   df_placebos = df_placebos_scpi_lasso,
   df_razao = df_razao_mspe_scpi_lasso,
   nome_variavel_y = "Investimento Real PPP (% do PIB)",
   ano_tratamento = nAnoTratamento, ano_inicial = nAnoInicial,
   ano_final = nAnoFinal, breaks_grafico = 2,
   paleta_trend = paleta_trend, paleta_gap = paleta_gap, 
   paleta_pesos = paleta_pesos, paleta_placebos = paleta_placebos
)

lista_graficos_scpi_lasso$pesos

lista_graficos_scpi_lasso$trends

lista_graficos_scpi_lasso$gap

lista_graficos_scpi_lasso$placebos

lista_graficos_scpi_lasso$razao_mspe

### VENCEDOR Demeaned - Ferman & Pinto (2021) ##################################
# MSPE: 9,77
# Preparando dados
dfNivel_InvTot_scpi_demeaned <- scdata(
   dfNivel %>% filter(Ano %in% c(nAnoInicial:nAnoFinalInv)), 
   id.var = "iso2c", time.var = "Ano",
   outcome.var = "Investimento_PIB_PPP", 
   period.pre = (2005:2016),
   period.post = (2017:2019), 
   unit.tr = "BR", unit.co = vDoadores[-1],
   constant = TRUE, cointegrated.data = TRUE
)

# Estimando
cs_InvTot_Nivel_scpi_demeaned <- scest(dfNivel_InvTot_scpi_demeaned, 
                                       w.constr = list(name = "simplex", Q = 1))

# Vendo resultados
summary(cs_InvTot_Nivel_scpi_demeaned)
scplot(cs_InvTot_Nivel_scpi_demeaned)

# Placebos
df_placebos_scpi_demeaned <- funcs$placebos_scpi(
   dados = dfNivel %>% filter(Ano %in% c(nAnoInicial:nAnoFinalInv)),
   string_coluna_unidade = "iso2c",
   string_coluna_tempo = "Ano",
   string_variavel_interesse = "Investimento_PIB_PPP",
   ano_tratamento = nAnoTratamento, unidade_tratada = "BR",
   ano_inicial = nAnoInicial, ano_final = nAnoFinalInv,
   restricao_w = list(name = "simplex", Q = 1),
   constant = TRUE, cointegrated = TRUE
)

## MSPE do Brasil pré-tratamento
(mspe_pre_br_scpi_demeaned <- funcs$mspe_pre_trat_scpi(cs_InvTot_Nivel_scpi_demeaned))

# Razão MSPE Pós-Tratamento
df_razao_mspe_scpi_demeaned <- funcs$razao_mspe_scpi(df_placebos_scpi_demeaned)
mspe_pos_scpi_demeaned <- mean(df_razao_mspe_scpi_demeaned$post_mspe[
   df_razao_mspe_scpi_demeaned$type == "donor"])

# Gráficos
lista_graficos_scpi_demeaned <- funcs$graficos_sc_scpi(
   objeto_sc_scpi = cs_InvTot_Nivel_scpi_demeaned,
   df_placebos = df_placebos_scpi_demeaned,
   df_razao = df_razao_mspe_scpi_demeaned,
   nome_variavel_y = "Investimento Real PPP (% do PIB)",
   ano_tratamento = nAnoTratamento, ano_inicial = nAnoInicial,
   ano_final = nAnoFinal, breaks_grafico = 2,
   paleta_trend = paleta_trend, paleta_gap = paleta_gap, 
   paleta_pesos = paleta_pesos, paleta_placebos = paleta_placebos,
   hjust_gap = .95
)

lista_graficos_scpi_demeaned$pesos
ggsave("Figuras/Investimento_Total/Pesos_InvTot_demeaned.pdf", dpi = 1200)

lista_graficos_scpi_demeaned$trends
ggsave("Figuras/Investimento_Total/Trends_InvTot_demeaned.pdf", dpi = 1200)

lista_graficos_scpi_demeaned$gap
ggsave("Figuras/Investimento_Total/Gap_InvTot_demeaned.pdf", dpi = 1200)

lista_graficos_scpi_demeaned$placebos
ggsave("Figuras/Investimento_Total/Placebo_InvTot_demeaned.pdf", dpi = 1200)

lista_graficos_scpi_demeaned$razao_mspe
ggsave("Figuras/Investimento_Total/Razao_MSPE_InvTot_demeaned.pdf", dpi = 1200)

