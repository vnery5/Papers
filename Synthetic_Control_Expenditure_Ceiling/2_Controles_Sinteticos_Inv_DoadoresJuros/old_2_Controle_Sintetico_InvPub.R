...
lista_graficos_ValoresY <- funcs$graficos_cs(
   ...
   nome_variavel_y = "Investimento Público PPP (% do PIB)", 
...
# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_PD_ValoresY <- funcs$mspe_placebos_pos_trat(cs_InvPub_PD_ValoresY, 
                                                         nAnoTratamento))
(mspe_pre_br_PD_ValoresY <- cs_InvPub_PD_ValoresY %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))
...
# Gráficos
lista_graficos_ValoresY <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPub_PD_ValoresY, 
   nome_variavel_y = "Investimento Público PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
...
## Pesos e médias
lista_graficos_ValoresY$pesos
funcs$tabela_media_cs(cs_InvPub_PD_ValoresY, stargazer = F)
...
lista_graficos_ValoresY$placebos
lista_graficos_ValoresY$razao_mspe
cs_InvPub_PD_ValoresY %>% grab_signficance()

### 3. Inflexões ####
# Controle Sintético
cs_InvPub_PD_Inflexoes <- dfPD %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_Publico_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
...
# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_PD_Inflexoes <- funcs$mspe_placebos_pos_trat(cs_InvPub_PD_Inflexoes, 
                                                          nAnoTratamento))
(mspe_pre_br_PD_Inflexoes <- cs_InvPub_PD_Inflexoes %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))
...
# Gráficos
lista_graficos_Inflexoes <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPub_PD_Inflexoes, 
   nome_variavel_y = "Investimento Público PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
...
## Pesos e médias
lista_graficos_Inflexoes$pesos
funcs$tabela_media_cs(cs_InvPub_PD_Inflexoes, stargazer = F)
...
lista_graficos_Inflexoes$placebos
lista_graficos_Inflexoes$razao_mspe
cs_InvPub_PD_Inflexoes %>% grab_signficance()


### 4. Primeiros 75% dos Valores de Y ####
cs_InvPub_PD_TresQuartosValoresY <- dfPD %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_Publico_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
...
# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_PD_TresQuartosValoresY <- funcs$mspe_placebos_pos_trat(
   cs_InvPub_PD_TresQuartosValoresY, nAnoTratamento
))
(mspe_pre_br_PD_TresQuartosValoresY <- cs_InvPub_PD_TresQuartosValoresY %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))
...
# Gráficos
lista_graficos_TresQuartosValoresY <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPub_PD_TresQuartosValoresY, 
   nome_variavel_y = "Investimento Público PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
...
## Pesos e médias
lista_graficos_TresQuartosValoresY$pesos
funcs$tabela_media_cs(cs_InvPub_PD_TresQuartosValoresY, stargazer = F)
...
lista_graficos_TresQuartosValoresY$placebos
lista_graficos_TresQuartosValoresY$razao_mspe
cs_InvPub_PD_TresQuartosValoresY %>% grab_signficance()

### 5. Últimos 25% dos Valores de Y ####
cs_InvPub_PD_UltQuartoValoresY <- dfPD %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_Publico_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
...
# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_PD_UltQuartoValoresY <- funcs$mspe_placebos_pos_trat(
   cs_InvPub_PD_UltQuartoValoresY, nAnoTratamento
))
(mspe_pre_br_PD_UltQuartoValoresY <- cs_InvPub_PD_UltQuartoValoresY %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))
...
# Gráficos
lista_graficos_UltQuartoValoresY <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPub_PD_UltQuartoValoresY, 
   nome_variavel_y = "Investimento Público PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
...
## Pesos e médias
lista_graficos_UltQuartoValoresY$pesos
funcs$tabela_media_cs(cs_InvPub_PD_UltQuartoValoresY, stargazer = F)
...
lista_graficos_UltQuartoValoresY$placebos
lista_graficos_UltQuartoValoresY$razao_mspe
cs_InvPub_PD_UltQuartoValoresY %>% grab_signficance()


### 6. Apenas Covariadas Externas ####
cs_InvPub_PD_Controles <- dfPD %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_Publico_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
...
# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_PD_Controles <- funcs$mspe_placebos_pos_trat(cs_InvPub_PD_Controles, 
                                                          nAnoTratamento))
(mspe_pre_br_PD_Controles <- cs_InvPub_PD_Controles %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))
...
# Gráficos
lista_graficos_Controles <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPub_PD_Controles, 
   nome_variavel_y = "Investimento Público PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
...
## Pesos e médias
lista_graficos_Controles$pesos
funcs$tabela_media_cs(cs_InvPub_PD_Controles, stargazer = F)
...
lista_graficos_Controles$placebos
lista_graficos_Controles$razao_mspe
cs_InvPub_PD_Controles %>% grab_signficance()


## Normalizados ----------------------------------------------------------------
### 2. Todos os Valores de Y ####
# Controle Sintético
cs_InvPub_Norm_ValoresY <- dfNorm %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_Publico_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
...
# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_Norm_ValoresY <- funcs$mspe_placebos_pos_trat(cs_InvPub_Norm_ValoresY, 
                                                      nAnoTratamento))
(mspe_pre_br_Norm_ValoresY <- cs_InvPub_Norm_ValoresY %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))
...
# Gráficos
lista_graficos_ValoresY <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPub_Norm_ValoresY, 
   nome_variavel_y = "Investimento Público PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
...
## Pesos e médias
lista_graficos_ValoresY$pesos
funcs$tabela_media_cs(cs_InvPub_Norm_ValoresY, stargazer = F)
...
lista_graficos_ValoresY$placebos
lista_graficos_ValoresY$razao_mspe
cs_InvPub_Norm_ValoresY %>% grab_signficance()

### 3. Inflexões ####
# Controle Sintético
cs_InvPub_Norm_Inflexoes <- dfNorm %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_Publico_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
...
# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_Norm_Inflexoes <- funcs$mspe_placebos_pos_trat(cs_InvPub_Norm_Inflexoes, 
                                                         nAnoTratamento))
(mspe_pre_br_Norm_Inflexoes <- cs_InvPub_Norm_Inflexoes %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))

# Gráficos
lista_graficos_Inflexoes_Norm <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPub_Norm_Inflexoes, 
   nome_variavel_y = "Investimento Público PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
...
## Pesos e médias
lista_graficos_Inflexoes_Norm$pesos
funcs$tabela_media_cs(cs_InvPub_Norm_Inflexoes, stargazer = F)

## Trends e Gaps
lista_graficos_Inflexoes_Norm$trends
lista_graficos_Inflexoes_Norm$gap

## Inferência
lista_graficos_Inflexoes_Norm$placebos
lista_graficos_Inflexoes_Norm$razao_mspe
cs_InvPub_Norm_Inflexoes %>% grab_signficance()


### 4. Primeiros 75% dos Valores de Y ####
cs_InvPub_Norm_TresQuartosValoresY <- dfNorm %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_Publico_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
...
# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_Norm_TresQuartosValoresY <- funcs$mspe_placebos_pos_trat(
   cs_InvPub_Norm_TresQuartosValoresY, nAnoTratamento
))
(mspe_pre_br_Norm_TresQuartosValoresY <- cs_InvPub_Norm_TresQuartosValoresY %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))
...
# Gráficos
lista_graficos_TresQuartosValoresY <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPub_Norm_TresQuartosValoresY, 
   nome_variavel_y = "Investimento Público PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
...
## Pesos e médias
lista_graficos_TresQuartosValoresY$pesos
funcs$tabela_media_cs(cs_InvPub_Norm_TresQuartosValoresY, stargazer = F)
...
lista_graficos_TresQuartosValoresY$placebos
lista_graficos_TresQuartosValoresY$razao_mspe
cs_InvPub_Norm_TresQuartosValoresY %>% grab_signficance()

### 5. Últimos 25% dos Valores de Y ####
cs_InvPub_Norm_UltQuartoValoresY <- dfNorm %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_Publico_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
...
# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_Norm_UltQuartoValoresY <- funcs$mspe_placebos_pos_trat(
   cs_InvPub_Norm_UltQuartoValoresY, nAnoTratamento
))
(mspe_pre_br_Norm_UltQuartoValoresY <- cs_InvPub_Norm_UltQuartoValoresY %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))
...
# Gráficos
lista_graficos_UltQuartoValoresY <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPub_Norm_UltQuartoValoresY, 
   nome_variavel_y = "Investimento Público PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
...
## Pesos e médias
lista_graficos_UltQuartoValoresY$pesos
funcs$tabela_media_cs(cs_InvPub_Norm_UltQuartoValoresY, stargazer = F)
...
lista_graficos_UltQuartoValoresY$placebos
lista_graficos_UltQuartoValoresY$razao_mspe
cs_InvPub_Norm_UltQuartoValoresY %>% grab_signficance()


### 6. Apenas Covariadas Externas ####
cs_InvPub_Norm_Controles <- dfNorm %>% 
   filter(Ano %in% c(nAnoInicial:nAnoFinalInv)) %>% 
   synthetic_control(outcome = Investimento_Publico_PIB_PPP,
                     unit = iso2c,
                     time = Ano,
...
# Vendo MSPE dos placebos pós-tratamento
(mspe_pos_Norm_Controles <- funcs$mspe_placebos_pos_trat(cs_InvPub_Norm_Controles, 
                                                       nAnoTratamento))
(mspe_pre_br_Norm_Controles <- cs_InvPub_Norm_Controles %>% 
      grab_signficance() %>% 
      filter(unit_name == "BR") %>% 
      pull(pre_mspe))
...
# Gráficos
lista_graficos_Controles <- funcs$graficos_cs(
   objeto_cs_tidy = cs_InvPub_Norm_Controles, 
   nome_variavel_y = "Investimento Público PPP (% do PIB)", 
   ano_tratamento = nAnoTratamento,
   ano_inicial = nAnoInicial, 
   ano_final = nAnoFinal, 
...
## Pesos e médias
lista_graficos_Controles$pesos
funcs$tabela_media_cs(cs_InvPub_Norm_Controles, stargazer = F)
...
lista_graficos_Controles$placebos
lista_graficos_Controles$razao_mspe
cs_InvPub_Norm_Controles %>% grab_signficance()
